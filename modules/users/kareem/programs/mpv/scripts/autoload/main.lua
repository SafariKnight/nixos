...
local function Split(list)
  ...
  while item_pos <= #list do
    ...
    item = item
      .. string.sub(list, item_pos, pos_before_escape + item_escape_count)
...
local function create_extensions()
  ...
  if o.videos then
    SetUnion(
      SetUnion(EXTENSIONS_VIDEO, EXTENSIONS_VIDEO_DEFAULT),
      o.additional_video_exts
    )
    SetUnion(EXTENSIONS, EXTENSIONS_VIDEO)
  end
  if o.audio then
    SetUnion(
      SetUnion(EXTENSIONS_AUDIO, EXTENSIONS_AUDIO_DEFAULT),
      o.additional_audio_exts
    )
    SetUnion(EXTENSIONS, EXTENSIONS_AUDIO)
  end
  if o.images then
    SetUnion(
      SetUnion(EXTENSIONS_IMAGES, EXTENSIONS_IMAGES_DEFAULT),
      o.additional_image_exts
    )
